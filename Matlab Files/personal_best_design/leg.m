...
link_lengths = [15; 50; 39.5; 55.8; 40.1; 40.3; 63.9; 37.7; 38.4; 49; 65.7];
...
function sel = choose_point(pts, prev, idx)
    if isempty(pts)
        % 如果没有交点，尝试回退到上一步估计以保持连续性（避免抛错）
        if ~any(isnan(prev))
            sel = prev;
            warning('choose_point: no intersections — using previous estimate for node %d.', idx);
            return;
        else
            % 无法恢复，返回 NaN 以便上层逻辑决定如何处理
            sel = [NaN, NaN];
            warning('choose_point: no intersections and no previous estimate for node %d; returning NaN.', idx);
            return;
        end
    end
    if any(isnan(prev))
